...
- [ ] 1.10 **DROPPED (v1 cut 2026-06-01) â€” Uptime Kuma Push Monitor deferred.** The in-VPS dead-man-switch is dropped together with the off-VPS one. Rationale: the operator already has VPS-level uptime monitoring on `n8n.datachefnow.com` which compensates for the dead-man-switch role at the host level; YAGNI on duplicating it inside the cluster. **Failure mode now uncovered:** a silent Schedule-Trigger non-fire (VPS up but n8n cron didn't tick) produces no alert; mitigation depends on the operator's existing host-level monitor catching adjacent symptoms (e.g. n8n process crash). Re-evaluate if a silent non-fire is ever observed in production.
...
- [x] 4.3 Write `backup-daily.sh`:
   ...
   - [ ] 4.3.12 **DROPPED (v1 cut 2026-06-01) â€” Kuma success push removed.** The `MYFINANCE_BACKUP_KUMA_PUSH_URL` env var is gone; no heartbeat is emitted on success. Remove this branch from `backup-daily.sh` if implemented.
...
- [ ] 7.4 **DROPPED (v1 cut 2026-06-01) â€” Watchdog workflow removed.** Rationale: removed together with Kuma since the operator's external VPS uptime monitoring covers the host-down case, and ErrorHandler still covers in-flight failures of Daily / PreOp. **Failure modes now uncovered:** (a) silent Schedule-Trigger non-fire of `MyFinanceBackup-Daily` (rare n8n bug, VPS up); (b) disaster-drill cadence reminder (no longer alerts at 400 days). For (b), the annual calendar reminder at Â§10.6 remains the cadence cue. Delete any committed `scripts/backup/n8n/MyFinanceBackup-Watchdog.json` before commit.
...
- [ ] 8.4 From the n8n UI, Import each `scripts/backup/n8n/*.json` file in order (FOUR files: Daily, PreOp, ErrorHandler, DispatchAlert); map credentials in n8n's Credentials UI: `MYFINANCE_BACKUP_AGE_IDENTITY` (identity for verify), `MYFINANCE_BACKUP_RUNNER_SECRET`, `MYFINANCE_BACKUP_NTFY_TOPIC`, **`MYFINANCE_BACKUP_RESEND_API_KEY`** (as a Header Auth credential or plain credential depending on how the HTTP Request node is configured). **Not present in v1 (cuts):** `MYFINANCE_BACKUP_GMAIL_APP_PASSWORD` (Gmail â€” Resend, 2026-06-01), `MYFINANCE_BACKUP_KUMA_PUSH_URL` (Kuma dropped, 2026-06-01), `MYFINANCE_PREOP_WEBHOOK_SECRET`, `MYFINANCE_BACKUP_HEALTHCHECKS_URL`.
...
- [ ] 8.5 **DROPPED (v1 cut 2026-06-01) â€” Kuma reachability test removed** (no Kuma in v1). Replacement smoke from inside the runner container: `docker exec myfinance-backup-runner curl -fsS -X POST https://api.resend.com/emails -H "Authorization: Bearer $MYFINANCE_BACKUP_RESEND_API_KEY" -H "Content-Type: application/json" -d '{"from":"alerts@datachefnow.com","to":"<inbox>","subject":"backup-runner smoke","text":"smoke"}'` returns `{"id":"..."}` 200; ntfy is already exercised by Â§9.6.
...
- [ ] 9.9 **DROPPED (v1 cut 2026-06-01) â€” Watchdog workflow removed.** Staleness detection no longer exists in v1; the equivalent end-to-end check is just Â§9.6 (forced failure produces ntfy + Resend) plus the operator's external VPS uptime monitor for the host-down case.
