...
- A **retention policy** for `daily/` (30 days) enforced by an R2 lifecycle rule so no manual housekeeping is required for the highest-churn prefix. **v1 (operator decision 2026-06-01):** the other prefixes (`weekly/`, `monthly/`, `pre-op/`, `quarantine/`) accumulate without expiry; rationale and spot-check cadence in Decision 3.
...
- **Dual-channel alerting** (ntfy.sh push + **Resend transactional email**) when the scheduled run fails, is skipped, or when restore verification fails. v1 cut (operator decision 2026-06-01): Gmail SMTP replaced with Resend (`alerts@datachefnow.com`) for production hygiene; Uptime Kuma in-cluster dead-man-switch dropped together with off-VPS one.
...
- **Class B (reads):** 1 `GetObject` for the verify pull + 1 `GetObject` for the post-upload SHA-256 re-verify (finding M12) + occasional ad-hoc `/status` peek by the operator (a handful of `GetObject` on the status JSON files — not on a schedule in v1 since Watchdog was dropped). Worst-case daily Class B: ~5 ops. Monthly: ~150 ops. **Headroom against the 10M/month Class B free tier: 99.998% unused.**
...
Lifecycle rules (v1, operator decision 2026-06-01): **`daily/` is the ONLY prefix with a bucket-side R2 lifecycle rule** (`myfinance-daily-30d`, 30-day deletion). The four other prefixes (`weekly/`, `monthly/`, `pre-op/`, `quarantine/`) accumulate without expiry. Rationale: YAGNI on a single-user system whose projected annual volume per prefix (weekly ≈ 2.6 GB/yr, monthly ≈ 0.6 GB/yr, pre-op ≈ 0.3 GB/yr, quarantine ≈ 0) sits inside R2's 10 GB free tier with the daily 30-day rule already capping the highest-churn prefix. **Compensating control:** the §10.5 monthly external-archive calendar reminder is augmented with an `rclone size r2:my-finance-view-backups/pre-op/` spot-check so accumulation stays visible; reinstating the four deferred rules is a 5-minute follow-up once accumulation becomes meaningful. The earlier 5-rule design (30/90/365/90/365 for daily/weekly/monthly/pre-op/quarantine) is deferred, not deleted from the design history.
...
- `MyFinanceBackup-Watchdog` — **DROPPED in v1 (operator decision 2026-06-01).** Rationale: removed together with Kuma since the operator's external uptime monitor on `n8n.datachefnow.com` covers the host-down case and the ErrorHandler covers in-flight failures of Daily / PreOp. Failure modes now uncovered: (a) silent Schedule-Trigger non-fire of Daily (rare n8n bug, VPS up); (b) disaster-drill cadence reminder (no longer alerts at 400 days — replaced by the annual calendar reminder per §10.6).
...
- `MyFinanceBackup-DispatchAlert` (sub-workflow) — Execute Workflow trigger taking `{title, body}` → ntfy HTTP Request POST + **Resend HTTP API POST** in parallel (both with Continue On Fail = true). Returns 2xx unconditionally so calling workflows do not double-alert. This is the **fourth and final** workflow file in `scripts/backup/n8n/` for v1 (Daily, PreOp manual-trigger, ErrorHandler, DispatchAlert — Watchdog dropped together with Uptime Kuma per Decision 7).
...
**Trade-off — same-VPS failure domain:** if the VPS itself is down (host outage, Docker crash, `apt-get upgrade --auto-remove` mishap, disk full), n8n + sidecar go down simultaneously and none of the in-cluster ntfy/Resend dispatches reach the operator. **Mitigation accepted for v1 (operator decision 2026-06-01):** the operator's external uptime monitor on `n8n.datachefnow.com` is the host-down signal; the parallel Gmail-ingest workflow's Telegram silence remains a secondary cue. Both Uptime Kuma (in-cluster dead-man-switch, v3) and healthchecks.io (off-VPS dead-man-switch, v2 M1) were considered and deferred — the external uptime monitor covers the same role with one fewer moving part.
...
### 7. Alerting: two independent channels (ntfy + Resend)

**Why two:** Each channel covers a separate failure-discovery context, and v1 deliberately accepts the dead-man-switch gap.

- **ntfy.sh + Resend transactional email (explicit dispatch)** answer "the run failed and we know it" — both fire in parallel from the worker as long as the runner has outbound internet.
- **No in-cluster dead-man-switch in v1 (operator decision 2026-06-01).** The Uptime Kuma Push Monitor and the off-VPS healthchecks.io pinger were both deferred. The operator's existing external uptime monitor on `n8n.datachefnow.com` is the host-down signal; a silent Schedule-Trigger non-fire while the VPS is up is uncovered (see "Failure modes" below).

**Channels:**

- **ntfy.sh**: HTTP Request node POSTs to `https://ntfy.sh/<topic>`. Topic name is an unguessable slug stored as n8n credential `MYFINANCE_BACKUP_NTFY_TOPIC` and surfaced as a placeholder in `.env.example`. Title is `MyFinance backup FAILED` / `verify FAILED` / `quarantined`; body is the last 20 lines of the script's log. Push priority `high` on failure.
- **Resend HTTP API**: n8n HTTP Request node POSTs to `https://api.resend.com/emails` with `Authorization: Bearer $MYFINANCE_BACKUP_RESEND_API_KEY` and JSON body `{"from":"alerts@datachefnow.com","to":"<operator-inbox>","subject":"<ntfy title>","text":"<full log>"}`. Domain `datachefnow.com` is already verified in Resend (Route 53 / us-east-1); the API key is scoped to "Sending access" only. Stored as n8n credential `MYFINANCE_BACKUP_RESEND_API_KEY` plus `MYFINANCE_BACKUP_ALERT_FROM` (sender) and `MYFINANCE_BACKUP_ALERT_TO` (recipient inbox) — all three surfaced in `.env.example` as placeholders. Body has space for the full log, not just the 20-line tail.

**Why Resend instead of Gmail SMTP (operator decision 2026-06-01):** production hygiene. The app will be customer-facing eventually; the operator does not want infrastructure email coming from a personal Gmail address. Resend separates sender identity (system domain) from recipient (operator inbox), avoids Google's "suspicious activity" lockouts when the VPS IP changes, and unifies email pathways with the rest of the future product (welcome emails, transaction notifications etc. will also use Resend). The M13 Gmail STARTTLS pinning fix is inapplicable to Resend's bearer-token model.

**VPS-wide failure-domain trade-off (v1 cut, operator decision 2026-06-01):** both alert channels are co-located on the operator's VPS. A complete VPS outage silences both simultaneously. **Mitigation accepted for v1:** the operator's external uptime monitor on `n8n.datachefnow.com` catches the host-down case; the parallel Gmail-ingest workflow's Telegram silence remains a secondary cue. **Failure modes now uncovered:** (a) silent Schedule-Trigger non-fire of Daily while VPS is up — no alert (Watchdog was the catch and has been dropped together with Kuma); detection path is the operator eyeballing R2 freshness. (b) Same-day quarantine-then-success visibility (finding #10 fix) still works because it is a worker-side artefact in `status/last-success.json`, not an alerting concern.

**Channels triggered when:**

| Event | ntfy | Resend |
|---|---|---|
| `backup-daily.sh` exits non-zero | ✓ | ✓ |
| `backup-preop.sh` exits non-zero | ✓ | ✓ |
| `verify-restore.sh` exits non-zero (chained from daily) | ✓ | ✓ |
| n8n ErrorHandler fires (workflow-engine error) | ✓ | ✓ |
| VPS-wide outage (n8n + sidecar both down) | — | — (operator notices via external uptime monitor on `n8n.datachefnow.com`) |
| Silent Schedule-Trigger non-fire (VPS up, n8n cron didn't tick) | — | — (uncovered in v1) |
| `backup-daily.sh` succeeds | — | — |

Successful daily runs upsert `r2://my-finance-view-backups/status/last-success.json` and append one line to `r2://…/status/status.log`. No alert on success.

**Alternatives considered (history retained for reviewer context):**

- **Uptime Kuma Push Monitor (in-cluster dead-man-switch, v3):** dropped in v1 (operator decision 2026-06-01) — operator's external monitor on `n8n.datachefnow.com` covers the host-down case the Kuma layer was meant to cover; YAGNI on duplicating it inside the cluster. Re-evaluate if a silent Schedule-Trigger non-fire is observed.
- **healthchecks.io (off-VPS dead-man-switch, v2 M1):** RETROACTIVELY DEFERRED in v3 then re-confirmed deferred in v1 (operator decision 2026-06-01). Same rationale: external uptime monitor covers the same role.
- **Gmail SMTP (v3):** replaced with Resend in v1 — production hygiene (see "Why Resend" above). Operator does not want personal Gmail to be the infrastructure sender once the app is customer-facing.
- Telegram via the existing n8n flow: tight coupling to the Gmail ingest workflow which itself depends on Supabase being healthy; if Supabase is the failure mode the Telegram path may be impaired. Rejected as the primary failure-alert channel; the operator notices `silence` on it as one of the VPS-outage cues, a different role.
...
| `GET` | `/status` | none | — | `200 {"lastSuccess":{...},"lastPreop":{...},"lastVerify":{...},"runInProgress":<bool>}` | Read from R2 `status/*.json` (M14 fix). **v1 (2026-06-01):** exposed for ad-hoc operator inspection; no scheduled in-cluster consumer in v1 (Watchdog dropped — see Decision 7). |
| `POST` | `/run/daily` | `X-Runner-Secret` | — | `200 {"artefact":"daily/<file>","sha256":"...","durationMs":...}` | Synchronous. Sidecar handles weekly/monthly promotion internally. **v1 (2026-06-01):** no Kuma success push (Decision 7). |
...
- **[VPS-wide outage → no backup AND no in-cluster alert]** → The VPS hosts n8n + sidecar simultaneously (v1 no longer includes Uptime Kuma per Decision 7), so a host outage silences both alerting channels (ntfy + Resend). **Mitigation accepted for v1 (operator decision 2026-06-01):** the operator's external uptime monitor on `n8n.datachefnow.com` catches the host-down case; the parallel Gmail-ingest workflow's Telegram silence within hours remains a secondary cue (typical day = 2–5 transactions arriving). An off-VPS dead-man-switch (healthchecks.io) was considered in v2 (M1 fix) and retroactively deferred in v3; an in-cluster Kuma dead-man-switch was added in v3 and dropped in v1 — both gone, external monitor is the substitute (see Decision 7 trade-off). Re-evaluate if a silent failure is ever observed.
...
- **[VPS is a single trust zone — n8n + sidecar share credentials, finding #9 fix]** → The secrets in `.env.local` + n8n credentials are co-located on the same VPS. A VPS root compromise (acknowledged in the bullet above) leaks ALL of: `BACKUP_DB_PASSWORD`, `BACKUP_R2_*`, `MYFINANCE_BACKUP_AGE_IDENTITY`, `MYFINANCE_BACKUP_RUNNER_SECRET`, the v1 alert credentials (`MYFINANCE_BACKUP_NTFY_TOPIC`, `MYFINANCE_BACKUP_RESEND_API_KEY`), AND grants ability to write new "trusted" snapshots. **Trust-boundary clarification:** the design does NOT claim defense-in-depth between the n8n container and the sidecar container — they share `n8n_net`, both can read each other's credentials if one is compromised via Node module supply-chain (the runner image) or via n8n's own attack surface (custom-node code injection). Treat the entire VPS as a single trust zone. Mitigations are *outside* the runner↔n8n split: VPS hardening, scope-of-token (R2 token scoped to a single bucket, Resend API key scoped to Sending-access on the verified domain — revocable instantly from the Resend dashboard without rotating other credentials). A future hardening path is moving `MYFINANCE_BACKUP_AGE_IDENTITY` to a hardware-backed credential (Yubikey, on-demand SSH agent forwarding from the operator's PC) so that decryption requires operator-physical-presence — deferred as a follow-up. Documented here so a future reader does not over-trust the runner↔n8n boundary.
...
- **[Resend API key leaks via n8n or sidecar compromise]** → The API key is scoped to "Sending access" only on the verified `datachefnow.com` domain, can be revoked instantly from the Resend dashboard, and is independent of the operator's primary credentials. Documented as a rotate-on-suspicion item. **v1 cut (2026-06-01):** replaces the Gmail App Password risk; the marginal change is that the blast radius of a leak is "attacker can send email purportedly from `alerts@datachefnow.com`" rather than "attacker can send email purportedly from `aftorresl01@gmail.com`" — strict improvement.
...
- **[Both alert channels fail simultaneously — silent failure]** → If ntfy.sh AND Resend both fail on the same dispatch (v1: two providers instead of three), the operator gets nothing on that dispatch. Lower-probability than under v2/v3's three-channel design but not zero. Mitigations: each channel has independent uptime, and a verify-failed pre-op MUST quarantine the artefact regardless of whether alerts delivered — so the operator at least sees the quarantine state next time they read `status/last-preop.json`. **v1 also accepts the gap that a silent Schedule-Trigger non-fire produces no in-cluster alert** (Watchdog dropped together with Kuma); the operator's external uptime monitor on `n8n.datachefnow.com` is the only cue for that failure mode.
...
2. **Cloudflare R2 setup**: Operator generates the R2 API token in the Cloudflare dashboard scoped to `my-finance-view-backups`, configures the **single** bucket lifecycle rule (`myfinance-daily-30d` deleting `daily/` after 30 days — v1, operator decision 2026-06-01), and records the account ID. The other four prefixes accumulate without expiry per Decision 3.
3. **Alerting credentials (v1, operator decision 2026-06-01)**: Operator creates a Resend API key with "Sending access" scope on the verified `datachefnow.com` domain (sender `alerts@datachefnow.com`), an unguessable ntfy.sh topic slug, and the runner shared-secret. **Not generated in v1:** Gmail App Password (replaced by Resend), Uptime Kuma push URL (Kuma dropped).
4. **Uptime Kuma (DROPPED in v1, operator decision 2026-06-01)** — no Push Monitor is created. The operator's existing external uptime monitor on `n8n.datachefnow.com` covers the host-down case; YAGNI on duplicating it inside the cluster. Skip this step.
...
7. **Import n8n workflows (v1)**: Operator imports the **four** n8n workflows from `scripts/backup/n8n/*.json` via the n8n UI Import feature (`Daily`, `PreOp`, `ErrorHandler`, `DispatchAlert` — no `Watchdog` in v1), then registers the n8n credentials: `MYFINANCE_BACKUP_AGE_IDENTITY` (identity for verify), `MYFINANCE_BACKUP_RUNNER_SECRET`, `MYFINANCE_BACKUP_NTFY_TOPIC`, **`MYFINANCE_BACKUP_RESEND_API_KEY`**. **Dropped credentials (v1, 2026-06-01):** `MYFINANCE_BACKUP_GMAIL_APP_PASSWORD` (replaced by Resend), `MYFINANCE_BACKUP_KUMA_PUSH_URL` (Kuma dropped).
...
9. **Trigger one daily cycle manually**: Operator runs `MyFinanceBackup-Daily` manually and confirms (a) artefact in `daily/`, (b) `last-success.json` updated, (c) `last-verify.json` updated with green probes. **v1:** no Kuma "Up" check (Kuma dropped).
10. **Activate schedules (v1)**: Operator sets the Schedule Trigger to Active on `MyFinanceBackup-Daily` only (no Watchdog in v1; PreOp stays manual-trigger). Waits one full 24-hour cycle and confirms the daily auto-run lands in R2 and `last-success.json` / `last-verify.json` update.
...
- **Reinstate an in-cluster (Kuma) or off-VPS (healthchecks.io) dead-man-switch after a silent failure is observed?** v1 (operator decision 2026-06-01) dropped BOTH layers — relies on the operator's external uptime monitor on `n8n.datachefnow.com` for host-down detection. If a silent Schedule-Trigger non-fire is ever observed in production, or if the external monitor proves insufficient (e.g. multi-day silence missed during travel), reinstating Kuma (in-cluster) or healthchecks.io (off-VPS) is a follow-up.
...
- **External + in-cluster dead-man-switch:** ADOPTED in v2 (M1 fix) THEN RETROACTIVELY DEFERRED in v3 (Gate C) THEN ALSO Kuma-layer dropped in v1 (operator decision 2026-06-01). v1 relies on the operator's external uptime monitor on `n8n.datachefnow.com` for host-down detection. Re-evaluate if a silent non-fire is observed. See Decision 7 and Open Questions.
...
- **Alerting email transport (Gmail SMTP vs Resend HTTP API):** RESOLVED in v1 (operator decision 2026-06-01) — Resend. Production hygiene: Resend separates infrastructure sender identity (`alerts@datachefnow.com`) from the operator's personal Gmail; avoids Google "suspicious activity" lockouts on VPS IP changes; unifies email transport with future product flows. See Decision 7.

- **R2 lifecycle policy count:** RESOLVED in v1 (operator decision 2026-06-01) — one rule only (`daily/` 30d). YAGNI for single-user system; the four other prefixes accumulate inside R2 free tier per the volume math in Decision 3. See Decision 3.

- **Pre-op webhook IP allowlist:** RE-RESOLVED in v1 (operator decision 2026-06-01) — no allowlist. The previous v3 cut had already removed the public webhook (manual n8n-UI trigger only); v1 confirms that the internal sidecar endpoint is protected by the 238-bit `MYFINANCE_BACKUP_RUNNER_SECRET` bearer with timing-safe compare, no further IP filtering. Rationale: the dynamic Colombian residential IP would lock the operator out mid-migration. See Decision 9.
...
- **M1 — Off-VPS dead-man-switch via healthchecks.io (defends against whole-VPS outage going silent).** Out of threat model: deferred in v1 because the operator's external uptime monitor on `n8n.datachefnow.com` covers the host-down case. The previous v3 Telegram-silence mitigation is now a secondary cue, not the primary signal. The Watchdog workflow + in-cluster Kuma Push Monitor that v3 retained were also dropped in v1 (operator decision 2026-06-01).
