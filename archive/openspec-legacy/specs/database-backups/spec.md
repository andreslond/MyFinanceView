...
## Purpose
Encrypted, scheduled, restore-verified backups of the Supabase Postgres remote (`myfinance` schema + `auth.users`) to Cloudflare R2, with on-demand pre-op snapshots before irreversible writes, multi-channel alerting on failure, and operator-facing status surfaces. Created by archiving change `supabase-backup-policy`.
