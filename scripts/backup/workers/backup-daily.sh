...
Decryption (primary identity — Windows PC or printed paper at location A):
  age -d -i %USERPROFILE%\\.config\\myfinance-backup\\age-identity-primary.txt ^
      -o snapshot.tar ${TODAY}.tar.age

Decryption (recovery identity — printed paper at location B):
  age -d -i recovery-identity.txt -o snapshot.tar ${TODAY}.tar.age
...
# ---------------------------------------------------------------------------
# 4.3.4 Two-recipient age encryption (B1 fix)
# ---------------------------------------------------------------------------
log_info "Encrypting with primary + recovery recipients"
age \
  -r "$(cat "${RECIPIENTS_DIR}/primary.txt")" \
  -r "$(cat "${RECIPIENTS_DIR}/recovery.txt")" \
...
# ---------------------------------------------------------------------------
# 4.3.13 healthchecks.io success ping (M1 fix, fire-and-forget)
# ---------------------------------------------------------------------------
HC_URL="${MYFINANCE_BACKUP_HEALTHCHECKS_URL:-}"
if [[ -n "$HC_URL" ]]; then
  curl -sS --max-time 10 "${HC_URL}" \
    > /dev/null 2>&1 || log_info "healthchecks.io ping failed (non-fatal)"
fi
